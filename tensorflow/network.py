...
class ResNetBaseModel(object):
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      ## caozhangjie add bottleneck layer
      hidden_features = tf.layers.dense(inputs=inputs, units=256)
      inputs = tf.layers.dense(inputs=hidden_features, units=self.num_classes)
      inputs = tf.identity(inputs, 'final_dense')
      return inputs, hidden_features
...
class ResNetModel(ResNetBaseModel):
  """Model class with appropriate defaults for Imagenet data."""

  def __init__(self, resnet_size, data_format=None, num_classes=31,
...
class AdversarialNetwork(object):
    ...
    def _model_variable_scope(self):
        return tf.variable_scope('adversarial_net',
                             custom_getter=self._custom_dtype_getter)
