...
def train(config):
    ...
    gradient_reverse_layer2 = network.AdversarialLayer(high_value=config["high"])
    if use_gpu:
    ...
    for i in range(config["num_iterations"]):
        ...
        features1, outputs1, features2, outputs2 = base_network(inputs)
        ...
        transfer_loss = loss.DANN(features1, ad_net1, gradient_reverse_layer1, use_gpu) \
                        + 0*loss.DANN(features2, ad_net2, gradient_reverse_layer2, use_gpu)

        classifier_loss = nn.CrossEntropyLoss()(outputs1.narrow(0, 0, inputs.size(0)//2), labels_source) + 0*nn.CrossEntropyLoss()(outputs2.narrow(0, 0, inputs.size(0)//2), labels_source)
